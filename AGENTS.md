...
**Queue:** `make queue:peek` (read-only), **`make queue:archive-top`** / **`make queue:archive`**, **`make queue:validate`**, then **`make queue:pr-merge`** (after archive — `gh` merge + delete branch for GitHub).
