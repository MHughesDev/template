...
**Queue:** `make queue:peek` (read-only), `make queue:validate`, **`make queue:pr-merge`** (merge open PR via `gh` + delete branch), **`make queue:archive-top`** (archive first open row — no id), `make queue:archive` where implemented.
