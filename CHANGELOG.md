...
- **`make queue:top-item`** — prints the first open row in `queue/queue.csv` as **one line** of JSON (all columns: `id`, `batch`, `phase`, `category`, `summary`, `dependencies`, `related_files`, `notes`, `created_date`). Agents must run this when processing queue work. Scripts: `scripts/queue_top_item.py`, `scripts/queue-top-item.sh`.
