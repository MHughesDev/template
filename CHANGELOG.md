...
- **Queue dependency-first insertion semantics** — clarified that when adding new rows to `queue/queue.csv`, maintainers must place each row after any open prerequisites listed in `dependencies`; dependency order takes precedence over FIFO/batch heuristics, then run `make queue:validate`.
...
- **MicroFast dev MCP** — stdio MCP server (`dev_mcp/`, `python -m dev_mcp`), Cursor `.cursor/mcp.json`, procedure [docs/procedures/microfast-dev-mcp.md](docs/procedures/microfast-dev-mcp.md), `mcp` dependency in `pyproject.toml`.
- **Full-stack baseline** — `apps/api/` (FastAPI + SQLModel + Postgres + Alembic) and `apps/web/` (React 19 + Vite + TanStack + Tailwind v4) ship as the deployable substrate that initialized products run on top of.
...
- **AGENTS.md §16 — Semantic discovery** — agents must use vector/similarity search (or equivalent) each turn to find relevant skills, procedures, prompts, and docs for the user query; complements mandatory skill index search. §3 workflow includes a discovery step. `spec/spec.md` §4.1 updated with items 15–16.
