...
5. **Optional — MicroFast dev MCP:** enable the `microfast-dev` MCP server in Cursor (`.cursor/mcp.json`) or run `./scripts/dev-mcp.sh` for stdio clients. It exposes queue helpers, `AGENTS.md` / skills index as resources, and canonical `make` targets — see [docs/procedures/microfast-dev-mcp.md](docs/procedures/microfast-dev-mcp.md).
