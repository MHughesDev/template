...
One paragraph. The queue seeder converts the human's work prioritization in idea.md §12 into properly formatted queue.csv rows with sequential IDs, batch assignments from phases (idea.md §15), and summaries that meet the minimum quality bar (≥100 chars with acceptance criteria).

## When to Invoke

During initialization Phase 5 (Seed Queue). After all profiles are configured. Before running make queue:validate.

## Prerequisites

idea.md §12 filled with at least one work item. idea.md §15 (phases) optional but used for batch assignment. queue/queue.csv exists with correct header.

## Relevant Files/Areas

idea.md §12 and §15, queue/queue.csv, queue/QUEUE_INSTRUCTIONS.md, skills/init/queue-seeder.py
...
Summary too short → make queue:validate fails. Fix: expand summary to include goal, acceptance criteria, definition of done.

## Handoff Expectations

queue.csv seeded, make queue:validate passes, first item ready for processing.

## Related Procedures

docs/procedures/initialize-repo.md (Phase 5)
...
.cursor/rules/queue.md (summary quality requirements)
