...
> CONTENT: One paragraph. Canonical commands reduce the chance of agents using wrong or inconsistent approaches. Every recurring multi-step operation deserves a named Make target or Cursor command. This skill ensures new commands are added consistently.

## When to Invoke

> CONTENT: When a recurring multi-step operation is identified that doesn't have a Make target. When adding a new Cursor command for an initialization or scaffolding workflow. When a script is written that should be exposed as a canonical target.

## Prerequisites

> CONTENT: The operation is clearly defined. A script in scripts/ exists or will be created. Makefile and docs/development/local-setup.md read.

## Relevant Files/Areas

> CONTENT: Makefile, scripts/README.md, docs/development/local-setup.md, .cursor/commands/, README.md
...
> CONTENT: `make help` (verify target appears)
...
> CONTENT: Duplicate target with different behavior → confusion. Fix: always read the full Makefile before adding.

## Handoff Expectations

> CONTENT: New target documented, help text accurate, script tested.
...
> CONTENT: .cursor/rules/global.md (canonical commands — ad hoc shell is disfavored)
