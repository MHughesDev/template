...
> CONTENT: One paragraph. Secrets in code are a critical security vulnerability. This skill establishes the complete policy for secret handling: where secrets live (env vars only), how to document them (.env.example), how to detect accidental leakage (scanner), and what to do when a secret is leaked (immediate rotation).
...
> CONTENT: .cursor/rules/security.md read. docs/security/secrets-management.md read. Access to the secret management system (password manager, vault, etc.).
...
> CONTENT: New secret properly documented, scanner passes, PR description confirms secret is documented in .env.example.

## Related Procedures

> CONTENT: docs/security/secrets-management.md, docs/security/incident-response.md
...
> CONTENT: .cursor/rules/security.md (Secret Sourcing Rules section)

## Machinery

> CONTENT: `skills/security/secret-scanner.py` — regex-based scanner for high-entropy strings and known secret patterns. Invoke: `python skills/security/secret-scanner.py --scan-path .` to scan all files, or `make security:scan` for the full security pipeline.
