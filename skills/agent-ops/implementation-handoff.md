...
> CONTENT: One paragraph. A handoff document is the evidence artifact that makes the next agent or human reviewer able to understand exactly what happened, verify the work, assess residual risks, and continue from where this agent stopped. It is non-optional for any meaningful change.
...
> CONTENT: The handoff document is a complete audit trail. A human reviewer should be able to: (1) understand what changed, (2) verify it worked (from commands output), (3) know what risks remain, (4) know what follow-up is needed — without talking to the agent.

## Related Procedures

> CONTENT: docs/procedures/handoff.md, docs/procedures/archive-queue-item.md

## Related Prompts

> CONTENT: prompts/implementation_agent.md, prompts/queue_processor.md

## Related Rules

> CONTENT: AGENTS.md §Navigation (handoff format), .cursor/rules/global.md (Evidence requirements)

## Machinery

> CONTENT: `skills/agent-ops/handoff-template-generator.py` — reads git diff, recent commands from shell history, queue.csv for current item, and generates a pre-filled Markdown handoff template. Invoke: `python skills/agent-ops/handoff-template-generator.py --queue-id Q-001`
