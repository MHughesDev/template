...
> CONTENT: The handoff is the agent's audit trail. A complete handoff enables the next agent or human reviewer to understand what happened without any additional context from the originating agent.

## Trigger / When to Use

> CONTENT: After all implementation is complete, before or alongside opening the PR. Also when stopping due to a blocker.

## Prerequisites

> CONTENT: Implementation done (or explicitly blocked). Validation run.

## Exact Commands

> CONTENT: `python skills/agent-ops/handoff-template-generator.py --queue-id <id>` — generate pre-filled template
...
> CONTENT: Complete handoff document in PR description. Queue notes updated with PR URL.
...
> CONTENT: If partial handoff (blocked): document what was done, what's remaining, what the blocker is. The partial handoff is still required.

## Handoff Expectations

> CONTENT: This procedure IS the handoff. Its output is the handoff document.
