...
> CONTENT: Skills encode recurring knowledge. Adding or updating them correctly ensures future agents can find and use the skill. A skill with missing sections is as bad as no skill.

## Trigger / When to Use

> CONTENT: When a recurring work pattern is identified (same class of task done 2+ times). When an existing skill needs to reflect changed tooling or commands.

## Prerequisites

> CONTENT: The skill need is clear. Correct category identified (agent-ops, backend, security, etc.). skills/README.md read.

## Exact Commands

> CONTENT: `make skills:list` (verify new skill appears), `make rules:check`
...
> CONTENT: New skill .md (and optional .py) file. Updated skills/README.md index.
...
> CONTENT: If skill format is wrong: refer to .cursor/rules/skills.md for required sections.

## Handoff Expectations

> CONTENT: New skill committed, index updated, rationale in PR.
